...
static void someTestFunc()
{
	...
	osmProcessor.matchNewPosition(pos, false);
...
void udpWorkLoop(PosReceiver &posReceiver)
{
	...
	inputPosition currPos;
	bool debugDumps = appConfigurator.getWriteDebugDumps();
	while (true)
	{
		currPos = positionReceiver.getCurrentPosition();
		if (currPos != prevPos)
		{
			osmProcessor.matchNewPosition(currPos, debugDumps);
			prevPos = currPos;
		}
